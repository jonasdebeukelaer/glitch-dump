...
def rand_affect_on_line_contrast(img: np.array) -> np.array:
    ...
    r_span = int(random.random() * 0.1 * scale)
    r_randomise = random.random() > 0.8
    r_if_contrast_less_than = random.random() > 0.7

    return affect_on_line_contrast(
        img, r_contrast, r_span, r_vertical, r_randomise, r_if_contrast_less_than
    )


def affect_on_line_contrast(
    img, contrast=10, span=10, vertical=False, randomise=False, less_than=True
):
    args = locals()
    del args["img"]
    print("args: ", args)
            ...
            if less_than == ((max_val - min_val) < contrast):
                if randomise:
                    ...
                    new_red = int(
                        1.0
                        * (
                            int(img[wrap(y_max, y), wrap(x_max, x - 1), 0])
                            + int(img[wrap(y_max, y), wrap(x_max - 1, x - 2), 0])
                            + int(img[wrap(y_max, y), wrap(x_max, x + 1), 0])
                            + int(img[wrap(y_max, y), wrap(x_max, x + 2), 0])
                        )
                        / 4
                    )
                    new_green = int(
                        1.0
                        * (
                            int(img[wrap(y_max, y), wrap(x_max, x - 1), 1])
                            + int(img[wrap(y_max, y), wrap(x_max - 1, x - 2), 1])
                            + int(img[wrap(y_max, y), wrap(x_max, x + 1), 1])
                            + int(img[wrap(y_max, y), wrap(x_max, x + 2), 1])
                        )
                        / 4
                    )
                    new_blue = int(
                        1.0
                        * (
                            int(img[wrap(y_max, y), wrap(x_max, x - 1), 2])
                            + int(img[wrap(y_max, y), wrap(x_max - 1, x - 2), 2])
                            + int(img[wrap(y_max, y), wrap(x_max, x + 1), 2])
                            + int(img[wrap(y_max, y), wrap(x_max, x + 2), 2])
                        )
                        / 4
                    )
                ...
                if new_red > 10 or new_green > 10 or new_blue > 10:
                    if img.shape[2] == 4:
                        new_img[y][max(x - span, 0) : min(x + span, x_max)][:] = [
                            new_red,
                            new_green,
                            new_blue,
                            255,
