...
def wavify_constant_wave(img, line_count, thickness=1, overlap=0, constant_wave_count=10) -> np.array:
...
def wavify_do(img, line_count, thickness=1, overlap=0, constant_wave=False, constant_wave_count=0) -> np.array:
    args = locals()
    del args["img"]
    print("args: ", args)
...
def wavify_monochrome(img, sparseness, thickness, overlap, offset, constant_wave, constant_wave_count) -> np.array:
    print("")
...
def wavify_circle(img: np.array, line_count: int, thickness: int, overlap: int) -> np.array:
    ...
    if sparseness < 1:
        raise ValueError('ratio of img res to line_count is too low, resulting sparseness = 0')

    for x, row in enumerate(img):
        display_percentage("running wavify_circle... ", x, img.shape[0])
        for y, px in enumerate(row):
            if on_concentric_circle(x, y, x_bar, y_bar, sparseness, thickness):
                new_x, new_y = new_coords(x, y, x_bar, y_bar, px[0], sparseness, overlap)
...
def on_concentric_circle(x: int, y: int, x_bar: int, y_bar: int, sparseness: int, thickness: int) -> bool:
...
def new_coords(x: int, y: int, x_bar: int, y_bar: int, value: int, sparseness: int, overlap: int) -> (int, int):
